...
#customize the functions that ggpais uses
...
# Lets do cleveland dot plots

cel_114 <- cel %>% filter(congress==114)
members <- sample_n(cel_114, 25)

ggplot(data = members) +
  aes(x = les, y = thomas_name) +
  geom_point()

ggplot(data = members) +
  aes(x = les, y = reorder(thomas_name, les)) +
  geom_point(size = 3.5) +
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
        panel.grid.major.y = element_line(linetype = 5,
                                          color = "red")) +
  labs(x = "Legislative effectiveness",
             y = NULL)



# Lets do a Lolipop figure
ggplot(data = members) +
  aes(x = reorder(thomas_name, les), y = les) +
  geom_point(size = 5, color = "red") +
  geom_segment(aes(x = thomas_name,
                   xend = thomas_name,
                   y = 0,
                   yend = les),
               size = 1.2,
